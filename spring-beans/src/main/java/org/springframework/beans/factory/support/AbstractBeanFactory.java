...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
		...
		if (sharedInstance != null && args == null) {
			...
			// 返回对象的实例，当实现了FactoryBean接口的对象，需要获取具体的对象的时候就需要此方法来获取
			...
			// 当对象都是单例的时候会尝试解决循环依赖的问题，但是多实例模式下如果存在循环依赖，则会抛出异常
			...
			if (parentBeanFactory != null && !containsBeanDefinition(beanName)) {
				...
				if (parentBeanFactory instanceof AbstractBeanFactory) {
					...
					// 如果有创建bean实例时要使用的参数，使用显示参数委托给父工厂
					// 使用父工厂获取bean对象，通过bean全类名和创建bean实例时要使用的参数
					return (T) parentBeanFactory.getBean(nameToLookup, args);
				}
				else if (requiredType != null) {
					// No args -> delegate to standard getBean method.
					// 没有创建bean实例时要使用的参数 -> 委托给标准的getBean方法
					// 使用父工厂获取bean对象，通过bean全类名和所需的bean类型
			...
			try {
				...
				// 检查mbd的合法性，不合格的引发验证异常
				checkMergedBeanDefinition(mbd, beanName, args);

				// Guarantee initialization of beans that the current bean depends on.
				// 获取当前Bean依赖的其他Bean，那么则优先实例化依赖的bean（如<bean id="person" class="com.xwl.debug.bean.Person" depends-on="teacher">，会先创建teacher对象）
				// 如果有依赖的bean，则按照getBean()把依赖的Bean先创建出来；
				...
				if (dependsOn != null) {
					...
					for (String dep : dependsOn) {
						...
						// 捕捉未找到BeanDefinition异常，'beanName'依赖于缺少的bean'dep'
				...
				if (mbd.isSingleton()) {
					...
					sharedInstance = getSingleton(beanName, () -> {
						try {
							...
							// 显示的从单例缓存中删除实例，他可能是由创建过程中急切的存放在那里，以允许循环引用解析，还要删除接收到该bean临时引用的任何bean
	...
	/**
	 * 该bean工厂是否包含动具有给定名称的bean定义成外部注册的singleton实例
	 *
	...
	public boolean containsBean(String name) {
		// 获取name最终的规范名称【最终别名】
		String beanName = transformedBeanName(name);
		// 如果beanName存在于singletonObjects【单例对象的高速缓存Map集合（一级缓存map）】中
		// 或者从beanDefinitionMap【Bean定义对象映射】中存在beanName的BeanDefinition对象
	...
	public boolean isFactoryBean(String name) throws NoSuchBeanDefinitionException {
		...
		Object beanInstance = getSingleton(beanName, false);
		if (beanInstance != null) {
			// 如果容器中存在，直接返回该bean是否为FactoryBean类型
	...
	/**
	 * BeanDefinition是否需要合并。BeanDefinition根据不同类型的配置文件信息，会将Bean封装到不同的Bean信息定义类中。
	 * 比如我们常用的配置文件版的GenericBeanDefinition；注解扫描版的ScannedGenericBeanDefinition等等。
	 * 而在这个过程中就出现了，父定义和子定义，我们需要在实际处理定义信息的时候进行合并处理
	 * 主要有以下三个方面：
	 * 1、存在父定义信息，使用父定义信息创建一个RootBeanDefinition，然后将自定义信息作为参数传入。
	 * 2、不存在父定义信息，并且当前BeanDefinition是RootBeanDefintion类型的，直接返回一份RootBeanDefintion的克隆
	 * 3、不存在父定义信息，并且当前BeanDefintion不是RootBeanDefintiton类型的，直接通过该BeanDefintion构建一个RootBeanDefintion返回
	 *
