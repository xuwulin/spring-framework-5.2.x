...
class ConfigurationClassParser {
	...
	protected void processConfigurationClass(ConfigurationClass configClass, Predicate<String> filter) throws IOException {
		 ...
		 * 如果此时父类为空，则不会进行while循环去解析，如果父类不为空，则会循环地去解析父类
		...
		// 将解析的配置类存储起来，这样回到parse方法时，能获取到值
	...
	protected final SourceClass doProcessConfigurationClass(
		...
		if (configClass.getMetadata().isAnnotated(Component.class.getName())) {
			...
			// 递归处理内部类，因为内部类也是一个配置类，配置类上有@Configuration注解，该注解继承@Component。if判断为true，调用processMemberClasses方法，递归解析配置类
			processMemberClasses(configClass, sourceClass, filter);
		}

		// Process any @PropertySource annotations
		// 如果配置类上加上了@PropertySource注解，那么就解析加载properties文件，并将属性添加到spring上下文中
		...
		// 处理@ComponentScan或者@ComponentScans注解，并将扫描包下的所有bean转换称填充后的ConfigurationClass
		// 此处就是将自定义的bean加载到IOC容器，因为扫描到的类可能也添加了@ComponentScan或@ComponentScans注解，因此需要递归解析
		...
		if (!componentScans.isEmpty() &&
			...
			for (AnnotationAttributes componentScan : componentScans) {
				 ...
				 * 比如 basePackages = com.xwl，那么在这一步会扫描出这个包及子包下的class，然后将其解析成BeanDefinition
				...
				/**
				 * 通过上一步扫描包com.xwl，有可能扫描出来的bean中可能也添加了@ComponentScan或者@ComponentScans注解，
				 * 所以这里需要循环遍历一次，进行递归(parse)，继续解析，知道解析出的类上没有@ComponentScan和@ComponentScans注解
				 */
		...
		// 解析父类，如果被解析的配置类继承了某个类，那么配置类的父类也会被解析
